...
- 顶部：**筛选（级联下拉）** + 全选本目录 / 清空选择 / 提交删除。
  - 下拉默认「全部分类」；**鼠标移到某分类的 `›` 会展开下一级**；点选某个分类即**单选该分类（含其子分类）**，按钮显示所选路径，**并在该项右侧显示 ✓ 与高亮**；点「全部分类」恢复全部。每项后的数字为**当前页面范围内、含子分类的合计**（文件管理=**当前目录递归下**的文件；视频审核=已截图；待删除=已选），与列表中显示的数量一致。
...
- **差异预览**：方案里始终包含 **“路径与分类不一致”** 清单（`/整理/X` 里却是分类 Y 的文件），网页「整理」页与命令预览都会显示数量与明细（`mismatch_count`）；**只有勾选“纠正”/加 `--fix-inside` 才会真正移动它们**。

...
- **交互式配置**：在向导菜单里选 `[4] 生成截图`，会先问**截图数量**（默认 50）和**并发数量**，并发不会超过本机 CPU 线程数。
- **实时进度**：运行时终端用**多行动态刷新**（原地重绘、不滚屏）：第一行是总进度条 `[████░░░░] 12/50 ✓10 ✗2  截图`，下面按并发数给**每个进行中的文件单独一行**，实时刷新其阶段（`取直链 / 直链就绪 / 截图 3/8`…）；结束后打印成功/失败汇总与失败文件名。输出不是终端（重定向到文件）时退化为单行进度条。

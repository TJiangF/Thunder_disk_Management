...
- 分类依据：工作室番号（JUQ/SONE/IPX/FSDSS…）、系列（FC2/Caribbean/1Pondo…）、盗版站域名（`hhd800.com`/`489155.com`/`aavv*`/`zzpp*`…）、站点关键词（brazzers/vixen/propertysex…）、中文特征（麻豆/探花/糖心/推特/淫妻…）、扩展名（pdf/zip/图片/音乐→非成人）。
- 纯命名推断，`未知` 需人工复核。
...
- **整理**：预览 organize 方案（移动清单 / 将删除的文件夹 / 保留的文件夹），点“重新生成方案”重算。**此页只预览，不改云盘**；真正执行用下方命令行的 `organize --apply`。
...
- **分类设置**：
  ...
  - **关键字规则**：为日本/欧美/国产/非成人各写自定义关键字（每行一个，子串匹配），或用“关键字=分类”做最高优先级覆盖；点“保存并重新分类”立即生效并刷新（写入 `data/classify_rules.json`，`./sweeper classify` 也会读取）。
...
- 顶部：**分类筛选 chips**（点击开关）+ 全选本目录 / 清空选择 / 提交删除。

...
- **整理**：预览 organize 方案（移动清单 / 将删除的文件夹 / 保留的文件夹）。顶部有**勾选框**：`执行移动` / `删除空的源文件夹` / `清理垃圾文件夹` / `含“成人-其他”` / `不自动重扫分类`，以及 `限量` 输入；勾好后点 **执行** 即在网页里真正执行。执行完会**自动重新扫描 + 分类**（可勾“不自动重扫分类”关闭），**进度条**显示 `[移动]/[清理]/[扫描]/[分类]` 各阶段进度，完成后页面自动刷新。也可用命令行 `organize --apply`。
...
- **只整理 `/整理` 之外的文件**：已经在 `/整理/...` 里的文件**不再被搬来搬去**（避免反复抖动）。

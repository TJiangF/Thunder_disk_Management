...
- **交互式配置**：向导菜单 `[4] 生成截图` 会先显示**当前已截图数量**，再问**本次新增数量**（默认 50）和**并发数量**；输入的数量是在已完成基础上**继续截**（等价 `--more N`），并发不会超过本机 CPU 线程数。
- **实时进度**：运行时终端用**多行动态刷新**（原地重绘、不滚屏），自上而下三块：
  1. **总进度条**（只统计本次目标数量）：`[████░░░░] 12/50 ✓10 ✗2  截图`；
  2. **每个线程一行**（共并发数行），显示该线程**当前正在截图的文件**及其阶段（`取直链 / 直链就绪 / 截图 3/8`…），一个文件截完立即刷新为下一个文件名；
  3. **日志区**（自动向下滚动，保留最近若干条）：打印异常与关键过程，如取链失败、某张超时、只截到一半等。
  结束后打印成功/失败汇总与失败文件名。输出不是终端（重定向到文件）时退化为单行进度条 + 逐行日志。
...
- **整理**：预览 organize 方案（移动清单 / 将删除的文件夹 / 保留的文件夹）。顶部有**勾选框**（移动默认执行）：`删除空白文件夹` / `清理垃圾文件夹` / `纠正 /整理 内错误归类`，以及 **执行** 按钮。执行完会**自动重新扫描 + 分类**，**进度条**显示 `[移动]/[清理]/[扫描]/[分类]` 各阶段进度，完成后页面自动刷新。也可用命令行 `organize --apply`。
